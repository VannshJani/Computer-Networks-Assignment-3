...
rtupdate1(struct rtpkt *rcvdpkt)
{
  ...
  if (updated) {
    ...
    if (dt1.costs[0][2] < mincost[0]) mincost[0] = dt1.costs[0][2];
    if (dt1.costs[2][0] < mincost[2]) mincost[2] = dt1.costs[2][0];
    if (dt1.costs[3][0] < mincost[3]) mincost[3] = dt1.costs[3][0];
    if (dt1.costs[3][2] < mincost[3]) mincost[3] = dt1.costs[3][2];
...
/* called when cost from 1 to linkid changes from current value to newcost*/
{
  ...
  printf("At time %f, linkhandler1() called for link to %d with newcost %d\n", 
         clocktime, linkid, newcost);
  
  /* Update the direct cost in the connectcosts array */
  connectcosts1[linkid] = newcost;
  
  /* Update the direct cost in the distance table */
  dt1.costs[linkid][linkid] = newcost;
  
  /* Recalculate minimum costs to all destinations */
  mincost[0] = dt1.costs[0][0];   /* Start with direct cost to node 0 */
  mincost[1] = 0;                 /* Cost to self is always 0 */
  mincost[2] = dt1.costs[2][2];   /* Start with direct cost to node 2 */
  mincost[3] = 999;               /* Start with infinity for node 3 */
  ...
  /* Send updates to neighbors regardless of changes (they need to know about the link cost change) */
  /* Send to node 0 */
  creatertpkt(&packet, 1, 0, mincost);
  tolayer2(packet);
  ...
  printf("At time %f, node 1 sends routing updates to neighbors due to link change\n", clocktime);
